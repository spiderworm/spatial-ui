	...
	function(
		...
		function PhysicsGameplayMode() {
			...
			Sun.prototype.__sendUpdate = function(update) {
				serviceMode.updateData({physics: {bodies: {sun: update}}});
			}
			...
			Teaatis.prototype.__sendUpdate = function(update) {
				serviceMode.updateData({physics: {bodies: {Teaatis: update}}});
			}
			...
			Ship.prototype.__tickPosition = function(seconds) {
				var throttle = serviceMode.getData('/physics/values/thrusters/forward');
			...
			Ship.prototype.__tickPitch = function(seconds) {
				var values = serviceMode.getData('/physics/values');
			...
			Ship.prototype.__sendUpdate = function(update) {
				serviceMode.updateData({physics: {bodies: {myShip: update}}});
			}
			...
			this.onDataReceived(function(data) {
				serviceMode.updateData(data);
			});

			this.onDataReceived('/physics/values/thrusters/kill',function(msg) {
				switch(msg) {
					case "forward":
						serviceMode.updateData('/physics/values/thrusters/forward',0);
					break;
					case "x":
						serviceMode.updateData('/physics/values/thrusters/pitch/throttle',0);
					break;
					case "y":
						serviceMode.updateData('/physics/values/thrusters/yaw/throttle',0);
					break;
					case "z":
						serviceMode.updateData('/physics/values/thrusters/roll/throttle',0);
					break;
				}
			});
		...
		PhysicsGameplayMode.id = "gameplay";
		PhysicsGameplayMode.prototype = new ServiceMode();
		PhysicsGameplayMode.prototype.constructor = PhysicsGameplayMode;

		return PhysicsGameplayMode;
